...
const BlogPost1 = () => {
  ...
  const paragraphs1 = firstBlog.info1.split('. ').map((sentence, index) => sentence.trim() + (index < firstBlog.info1.split('. ').length - 1 ? '.' : ''))
  const paragraphs2 = firstBlog.info2.split('. ').map((sentence, index) => sentence.trim() + (index < firstBlog.info2.split('. ').length - 1 ? '.' : ''))
    const otherBlogs = blogInfo.filter((blog) => blog.id !== firstBlog.id)
      ...
      <div className='flex justify-center -translate-y-65 items-center w-[40%] bg-black h-auto p-5 text-white'>
        <div className='flex flex-col gap-3 border border-gray-700 w-full h-full px-15 py-9'>
          <div className='flex justify-between text-[#8a8a8a] text-sm mb-5'>
            <p>
              {firstBlog.date} · {firstBlog.readTime} min read
            </p>
          ...
          <div className='flex flex-col gap-3'>
            <h1 className='text-5xl'>{firstBlog.title}</h1>
          ...
          <div className='w-full'>
            <img src={firstBlog.img1} alt="" className='w-full' />
            <p className='text-sm italic'>{firstBlog.photoMembers1}</p>
          </div>
          <div className='flex flex-col gap-3'>
            ...
            <a href='https://engineering.cmu.edu/afretec/index.html' className='text-[#FD6262]'>https://engineering.cmu.edu/afretec/index.html</a>
          </div>
          ...
          <div className='flex justify-between text-[#8a8a8a] text-sm'>
            ...
            <div
              ...
              onClick={() => toggleLike(firstBlog.id)}
            >
              <p>{likedBlogs[firstBlog.id] ? '1' : '0'}</p>
              <HeartIcon fill={likedBlogs[firstBlog.id] ? 'red' : 'none'} stroke="red" />
            </div>
          </div>
        </div>
      </div>
