...
const BlogPost2 = () => {
  ...
  const paragraphs1 = secondBlog.info1.split('. ').map((sentence, index) => sentence.trim() + (index < secondBlog.info1.split('. ').length - 1 ? '.' : ''))
  const paragraphs2 = secondBlog.info2.split('. ').map((sentence, index) => sentence.trim() + (index < secondBlog.info2.split('. ').length - 1 ? '.' : ''))
  const paragraphs3 = secondBlog.info3.split('. ').map((sentence, index) => sentence.trim() + (index < secondBlog.info3.split('. ').length - 1 ? '.' : ''))

  const otherBlogs = blogInfo.filter((blog) => blog.id !== secondBlog.id)
      ...
      <div className='flex justify-center -translate-y-65 items-center w-[40%] bg-black h-auto p-5 text-white'>
        <div className='flex flex-col gap-3 border border-gray-700 w-full h-full px-15 py-9'>
          <div className='flex justify-between text-[#8a8a8a] text-sm mb-5'>
            <p>
              {secondBlog.date} · {secondBlog.readTime} min read
            </p>
          ...
          <div className='flex flex-col gap-3'>
            <h1 className='text-5xl'>{secondBlog.title}</h1>
          ...
          <div className='w-full'>
            <img src={secondBlog.img1} alt="" className='w-full' />
            <p className='text-sm italic text-[#8a8a8a]'>{secondBlog.photoMembers1}</p>
          </div>
          <div className='flex flex-col gap-3'>
          ...
          <div className='w-full'>
            <img src={secondBlog.img2} alt="" className='w-full' />
            <p className='text-sm italic text-[#8a8a8a]'>{secondBlog.photoMembers2}</p>
          </div>
          <div className='flex flex-col gap-3'>
          ...
          <div className='flex justify-between text-[#8a8a8a] text-sm'>
            ...
            <div
              ...
              onClick={() => toggleLike(secondBlog.id)}
            >
              <p>{likedBlogs[secondBlog.id] ? '1' : '0'}</p>
              <HeartIcon fill={likedBlogs[secondBlog.id] ? 'red' : 'none'} stroke="red" />
            </div>
          </div>
        </div>
      </div>
